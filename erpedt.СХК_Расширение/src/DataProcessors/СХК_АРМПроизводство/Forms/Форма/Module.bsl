...
Процедура Группа7ПриСменеСтраницы(Элемент, ТекущаяСтраница) 
	...
	ИначеЕсли ТекущаяСтраница.Имя = "ГруппаВозврат" Тогда
		ГруппаВозвратНаСервере();
		Элементы.СписокСМ.Обновить();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ГруппаВозвратНаСервере()  
	ТаблицаСМ.Очистить();
	
	масВидНоменклатуры = Новый СписокЗначений;
	масВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Материалы: Зап. части").Ссылка);
	масВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.ПолучитьСсылку(Новый УникальныйИдентификатор("3193fdc9-ec87-11e7-9290-001e67f91767")).Ссылка);
	масВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Полуфабрикаты (действующие)").Ссылка);
	масВидНоменклатуры.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Полуфабрикаты собственного производства").Ссылка);
	
	СписокСМ.Параметры.УстановитьЗначениеПараметра("ТекДата",КонецДня(Период));		
	СписокСМ.Параметры.УстановитьЗначениеПараметра("Склад",Объект.СХК_Склад);		
	СписокСМ.Параметры.УстановитьЗначениеПараметра("Помещение",Объект.СХК_Этаж);		
	СписокСМ.Параметры.УстановитьЗначениеПараметра("ВидНоменклатуры",масВидНоменклатуры);	
...
&НаСервере
Функция СоздатьДокументыСМНаСервере()
	...
	//
		...
		Док = Документы.ЗаказНаПеремещение.СоздатьДокумент();
		Док.Дата = ТекущаяДата(); 
		Док.Организация = Справочники.Организации.НайтиПоРеквизиту("ИНН","3309000717");
		Док.СкладОтправитель = Объект.СХК_Склад ;
		Док.СкладПолучатель = Справочники.Склады.НайтиПоНаименованию("СМ Склад сырья и материалов"); 
		Док.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваров;
		Док.Приоритет = Справочники.Приоритеты.НайтиПоНаименованию("Средний"); 
		Док.ВариантПриемкиТоваров = Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным;
		Док.СпособДоставки = Перечисления.СпособыДоставки.Самовывоз;
		...
		Для Каждого стр из ТаблицаСМ Цикл
			Т = Док.Товары.Добавить();
			ЗаполнитьЗначенияСвойств (Т,стр);
			Т.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить;
			Т.НачалоОтгрузки = ТекущаяДата(); 
			Т.Серия  = Справочники.СерииНоменклатуры.ПустаяСсылка();
			Т.СтатусУказанияСерий = 0;
		...
		Исключение
			Док.Записать(РежимЗаписиДокумента.Запись);   
			Результат.ТекстОшибки = "Ошибка создания заказ на перемещение";
            Возврат Результат;
		КонецПопытки;
		/////////////// Расходный ордер 
		//СтруктураПараметровСозданияРасходныхОрдеров
		СтруктураПараметров = Новый Структура;
		СтруктураПараметров.Вставить("Получатель",Справочники.Склады.НайтиПоНаименованию("СМ Склад сырья и материалов"));
		СтруктураПараметров.Вставить("Ответственный",Пользователи.АвторизованныйПользователь());
		СтруктураПараметров.Вставить("Помещение",Объект.СХК_Этаж);
		...
		СтруктураПараметров.Вставить("Склад",Объект.СХК_Склад); 
		...
		Если РасхОрдер.ОформленныеОрдера.Количество() > 0 Тогда
			...
			РОрдер.Ответственный = Пользователи.АвторизованныйПользователь();
		...
		КонецЦикла;
			...
			Попытка
				...
				Результат.Успех = Истина; 
				Результат.ДокументРО = РОрдер.Ссылка;
			Исключение
				РОрдер.Записать(РежимЗаписиДокумента.Запись);
				Результат.ТекстОшибки = "Ошибка создания расходный ордер";
            	Возврат Результат;
	...
	//	
		...
		СтруктураП.Вставить("Статус",Перечисления.СтатусыПеремещенийТоваров.Отгружено);
		...
		РеквизитыШапки.Вставить("Организация",Справочники.Организации.НайтиПоРеквизиту("ИНН","3309000717"));
		РеквизитыШапки.Вставить("СкладОтправитель",Объект.СХК_Склад);
		РеквизитыШапки.Вставить("СкладПолучатель",Справочники.Склады.НайтиПоНаименованию("СМ Склад сырья и материалов"));//Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции"));
		...
		РеквизитыШапки.Вставить("Автор",Пользователи.АвторизованныйПользователь());
		РеквизитыШапки.Вставить("Ответственный",Пользователи.АвторизованныйПользователь());
		...
		Исключение  
			ДокП.Записать(РежимЗаписиДокумента.Запись);  
				Результат.ТекстОшибки = "Ошибка создания перемещения";
            	Возврат Результат;
...
&НаКлиенте
Процедура СоздатьДокументыСМ(Команда) 
	
	РезультатСервера = СоздатьДокументыСМНаСервере();
	
	Если НЕ РезультатСервера.Успех Тогда 
		
	Иначе
		ТабДок = СформироватьВнешнююПечатнуюФорму(РезультатСервера.ДокументРО); 
		ТабДок.Показать("Расходный ордер");			
	КонецЕсли;
	
		
	//СоздатьДокументыСМНаСервере();
	Элементы.СписокСМ.Обновить();
КонецПроцедуры
...
Функция СформироватьВнешнююПечатнуюФорму(СсылкаНаДокумент)
	...
	СсылкаНаВПФ = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоНаименованию("Расходный ордер на товары (внешняя QR-Код)");
	...
	ПараметрыИсточника.Вставить("ИдентификаторКоманды", "ПФ_РасходныйОрдерНаТоварыВнешняяФормаQR");
...
&НаКлиенте
Процедура ОбъектСХК_Этаж1ПриИзменении(Элемент)
		ГруппаВозвратНаСервере();
		Элементы.СписокСМ.Обновить();
КонецПроцедуры

&НаКлиенте
Процедура СписокСМВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)  
	
	СтандартнаяОбработка = Ложь;
	ТекСтрока = Элемент.ТекущиеДанные;  
	Если ТекСтрока.КоличествоУпаковок > 0 Тогда
		ПараметрыФормыОстатки = Новый Структура;
		ПараметрыФормыОстатки.Вставить("Команда","Добавить" );
		ПараметрыФормыОстатки.Вставить("ТекСтрока",ТекСтрока );
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыКоличество", ЭтаФорма);
		ОткрытьФорму("Обработка.СХК_АРМПроизводство.Форма.ФормаКоличество", _Параметры,ЭтаФорма, , , , ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждениеКоличества", ЭтотОбъект);	
		ПоказатьПредупреждение(Оповещение,Строка("Для отгрузки не достаточно количества !!!"),5,"Внимание!!!");
	КонецЕсли;
...
&НаКлиенте
Процедура ПослеЗакрытияПредупреждениеКоличества(Значение) Экспорт 
	
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьСтрокуТЗ(СЗ)
	Т = ТаблицаСМ.Добавить();
	ЗаполнитьЗначенияСвойств (Т,СЗ); 	
КонецПроцедуры


&НаКлиенте
Процедура ОбработатьЗакрытиеФормыКоличество(Значение, ДопПараметры) Экспорт
	Если ЗначениеЗаполнено(Значение) Тогда
		Если Значение.Количество	> 0 Тогда
			ДобавитьСтрокуТЗ(Значение);
		КонецЕсли;
	КонецЕсли;	
