...
&НаСервере
Функция НаборПродаж()
	СтрайпЛента = Справочники.Номенклатура.НайтиПоНаименованию("Страйп-лента Батончики");
	...
	Запрос.Текст ="ВЫБРАТЬ
	              ...
	              |ГДЕ
	              |	(ДвиженияСерийТоваров.Серия.СХК_Префикс = &СХК_Префикс ИЛИ ДвиженияСерийТоваров.Номенклатура = &СтрайпЛента)
	...
	Запрос.УстановитьПараметр("СтрайпЛента",СтрайпЛента);  
	...
	Для Каждого стр из Таб Цикл 
		Если стр.Номенклатура = СтрайпЛента Тогда
			стр.Количество = стр.Количество*25;	
			стр.Номенклатура = Справочники.Номенклатура.НайтиПоНаименованию("Протеиновый батончик - Банан в темной глазури, SmartBar Protein 40 г.");	
			стр.Персонаж = "Шайбу! Шайбу!";	
			стр.СуммаВыручкиБезНДС = стр.Количество * 38.42;	
			стр.Цена = 38.42;	
			
			Т = Таб.Добавить(); 
			ЗаполнитьЗначенияСвойств(Т,стр);
			Т.Номенклатура = Справочники.Номенклатура.НайтиПоНаименованию("Протеиновый батончик - Двойной шоколад в темной глазури, SmartBar Protein 40 г.");	
...
// Формирует данные по продажам продукции серии "СМФ" для контрагентов за заданный период.
//
// Возвращаемое значение:
//   ТаблицаЗначений:
//     * Количество - Число - суммарное количество проданной продукции
//     * СуммаВыручкиБезНДС - Число - сумма выручки без учета НДС
//     * Получатель - СправочникСсылка.Контрагенты - контрагент-получатель товара
